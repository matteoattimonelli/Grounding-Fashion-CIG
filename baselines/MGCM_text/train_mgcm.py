...
def train_step(
    ...
    for batch in loop:
        tshirts = batch['top'].to(device)
        true_pants = batch['bottom'].to(device)
        ...
        enc_A, enc_Bg, fake_B = generator(tshirts, t_emb.detach())
        real_B, _ = discriminator(tshirts, true_pants, t_emb.detach())

        fake_rec_B, _ = discriminator(tshirts, fake_B, t_emb.detach())
        ...
        enc_A, enc_Bg, fake_B = generator(tshirts, t_emb)
        fake_rec_B, _ = discriminator(tshirts, fake_B, t_emb.detach())
        ...
        with torch.no_grad():
            t_emb = text_model(**t_inputs).last_hidden_state[:, 0, :]
        enc_A, enc_Bg, fake_B = generator(tshirts, t_emb)
...
if __name__ == '__main__':
    ...
    with open(args.out_csv, "a") as log_file:
        log_file.write("alpha,beta,mi,ni,mrr10,mrr50,r10,r50,n10,n50\n")
    log_file.close()

    for a, b, m, n in tqdm(hyperparameter_grid, desc='GRID', leave=False):
        with open(args.out_csv, "a") as log_file:
            mrr10, mrr50, r10, r50, n10, n50 = train(a, b, m, n, args)
            log_file.write(f"{a},{b},{m},{n},{mrr10},{mrr50},{r10},{r50},{n10},{n50}\n")
        log_file.close()
        torch.cuda.empty_cache()
