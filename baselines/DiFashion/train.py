...
def parse_all_args():
    ...
    parser.add_argument('--dataset', choices=['fashionvc', 'expreduced', 'fashiontaobaoTB'], required=True)
    # parser.add_argument('--dataset', choices=['fashionvc', 'expreduced', 'fashiontaobaoTB'], default='fashionvc')
    parser.add_argument('--device', type=str, help="device.", default='cuda')
    parser.add_argument('--img_size', type=int, help="Image Size.", default=512)
    parser.add_argument(
...
def main():
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            if args.use_ema:
                load_model = EMAModel.from_pretrained(os.path.join(input_dir, "unet_ema"), UNet2DConditionModel, use_safetensors=False)
                ema_unet.load_state_dict(load_model.state_dict())
                ema_unet.to(device)
                del load_model
            
            if args.use_ema_fashion:
                load_model = EMAModel.from_pretrained(os.path.join(input_dir, "fashion_encoder_ema"), MutualEncoder, use_safetensors=False)
            ...
            for i in range(len(models)):
                ...
                load_model = MutualEncoder.from_pretrained(input_dir, subfolder="fashion_encoder", use_safetensors=False)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(diffusion):
                ...
                loss = diffusion(batch, null_img, weight_dtype)
