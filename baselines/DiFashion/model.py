...
def _candidate_repo_ids(repo_id: str) -> list[str]:
    candidates = [repo_id]
    if repo_id.startswith("stabilityai/"):
        suffix = repo_id.split("/", 1)[1]
        candidates.append(f"sd2-community/{suffix}")
        candidates.append(f"sd-research/{suffix}")
    return candidates


def _load_with_repo_fallback(loader_cls, repo_id: str, **kwargs):
    last_error = None
    for candidate in _candidate_repo_ids(repo_id):
        try:
            return loader_cls.from_pretrained(candidate, **kwargs)
        except Exception as exc:
            last_error = exc
            print(f"[DiFashion] failed loading {loader_cls.__name__} from {candidate}: {exc}")
    raise last_error


def extract_type_output(input_prompt: str) -> str | None:

    """

    Extracts the value associated with "Type" using regex,

    handling variations like "[Type]:", "Type:", or ",Type:".
    ...
    # ^        : Anchors the match to the beginning of the (stripped) line.

    # [\[,]?   : Optionally matches a literal '[' or a ','. (For "[Type]" or ",Type")

    # Type     : Matches the literal string "Type".

    # \]?      : Optionally matches a literal ']'. (For "[Type]")

    # :        : Matches the literal colon.

    # \s*      : Matches zero or more whitespace characters.

    # (.*)     : Captures any characters following (the value). This is group 1.

    #

    # This pattern is designed to match lines like:
    ...
    # even with varied spacing, e.g., "Type:Value" or "  [Type]:  My Value  "

    # Pre-compile the regex for slight efficiency if this function were called many times,

    # though for single calls, the benefit is negligible. It's good practice.

    type_pattern = re.compile(r"^[\[,]?Type\]?:\s*(.*)")

    for line in input_prompt[0].splitlines():
    # for line in input_prompt.splitlines():

        # Strip leading/trailing whitespace from the line itself

        # to ensure the '^' in regex works as expected on the content.

        cleaned_line = line.strip() 

        match = type_pattern.search(cleaned_line)

        if match:

            # group(1) is the (.*) part, which is the value.

            # Strip whitespace from the extracted value as well.
...
class DiFashion(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        base_model_id = getattr(args, "pretrained_model_name_or_path", "stabilityai/stable-diffusion-2-1-base")
        scheduler_model_id = getattr(args, "scheduler_model_id", base_model_id)
        text_model_id = getattr(args, "text_model_id", base_model_id)
        vae_model_id = getattr(args, "vae_model_id", base_model_id)

        self.noise_scheduler = _load_with_repo_fallback(PNDMScheduler, scheduler_model_id, subfolder="scheduler")
        ...
        # )
        self.tokenizer = _load_with_repo_fallback(
            CLIPTokenizer, text_model_id, subfolder="tokenizer", revision=args.revision
        )
        # logger.info("load CLIPTextModel...")
        # self.text_encoder = CLIPTextModel.from_pretrained(
        #     args.pretrained_model_name_or_path, subfolder="text_encoder", revision=args.revision
        # )
        self.text_encoder = _load_with_repo_fallback(
            CLIPTextModel, text_model_id, subfolder="text_encoder", revision=args.revision
        )
        ...
        self.vae = _load_with_repo_fallback(AutoencoderKL, vae_model_id, subfolder="vae", revision=args.revision)
        self.vae_scale_factor = 2 ** (len(self.vae.config.block_out_channels) - 1)
        # logger.info("load UNet...")
        self.unet = _load_with_repo_fallback(
    ...
    def forward(self, batch, null_img, weight_dtype):
        # uids = batch["uids"]
        ...
        if len(prompt) > 1:
            new_prompt = []
            for p in prompt:
                cat = extract_type_output(p)
                if cat is not None:
                    p = "A photo of a " + cat + ", on white background, high quality"
                else:
                    p = "A photo of a bottom garment on white background, high quality"
                new_prompt.append(p)
        else:
            cat = extract_type_output(prompt)
            if cat is not None:
                p = "A photo of a " + cat + ", on white background, high quality"
            else:
                p = "A photo of a bottom garment on white background, high quality"
        ...
        # cat = extract_type_output(prompt)
        # if cat is not None:
        #     prompt = "A photo of a " + cat + ", on white background, high quality"
        # else:
        #     prompt = "A photo of a bottom garment on white background, high quality"
        
        prompt = new_prompt

        top_img = batch['top'].to(self.device)
        bottom_img = batch['bottom'].to(self.device)

        prompt_tok = self.tokenizer(prompt, max_length=self.tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt")
        category_prompts = self.text_encoder(prompt_tok.input_ids.to(self.device))[0].to(dtype=self.text_encoder.dtype, device=self.device)
        ...
        # latents = self.vae.encode(top_img.to(weight_dtype)).latent_dist.sample() ## essendo questo il mio target, qui ci metto il bottom
        latents = self.vae.encode(bottom_img.to(weight_dtype)).latent_dist.sample() ## essendo questo il mio target, qui ci metto il bottom
        ...
        if self.args.use_mutual_guidance:
            ...
            mutual_cond = self.vae.encode(top_img.to(weight_dtype)).latent_dist.sample()
            # mutual_cond = latents.to(self.device)
            mutual_cond = self.fashion_encoder(mutual_cond) ## essendo questo il conditioning, qui ci metto il top. 
        else:
            # mutual_cond = torch.stack([null_latent] * (bsz * olen))
            mutual_cond = torch.stack([null_latent] * bsz).to(self.device)
    ...
    def fashion_generation(
        ...
        fill_cate = 1
        hist_latents = []
        for i,cate in enumerate(range(fill_cate)):
        ...
        top_latents = self.vae.encode(top_img).latent_dist.mode()[0] * self.vae.config.scaling_factor
        ...
        for i, t in enumerate(timesteps):
            ...
            if self.args.use_mutual_guidance:
                ...
                mutual_cond = top_latents.to(self.device)
                # mutual_cond = torch.stack(mutual_cond).to(self.device)
                mutual_cond = self.fashion_encoder(mutual_cond[None])
            else:
    ...
    def fashion_generation_no_cat(
        ...
        prompt_tok = self.tokenizer(prompt, max_length=self.tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt")
        category_prompts = self.text_encoder(prompt_tok.input_ids.to(self.device))[0].to(dtype=self.text_encoder.dtype, device=self.device)
        fill_num = category_prompts.shape[0]
        ...
        # null_img = null_img
        null_latent = self.vae.encode(null_img).latent_dist.mode() * self.vae.config.scaling_factor

        # No per-user history is used in this simplified generator, so the
        # history branch is just the null latent replicated to the current batch.
        hist_latents = null_latent.to(self.device)
        
        if do_classifier_free_guidance:
            null_hist_latents = null_latent
            hist_latents = torch.cat([hist_latents, null_hist_latents, null_hist_latents, null_hist_latents], dim=0)
        elif do_classifier_free_guidance_category:
            if do_classifier_free_guidance_hist:
                null_hist_latents = null_latent
        ...
        top_latents = self.vae.encode(top_img).latent_dist.mode() * self.vae.config.scaling_factor
        ...
        for i, t in enumerate(timesteps):
            ...
            if self.args.use_mutual_guidance:
                ...
                #     weighted_latents = sum([weight * emb for weight,emb in zip(weights, mutual_latents)])
                #     mutual_cond.append(weighted_latents)
                mutual_cond = self.fashion_encoder(top_latents.to(self.device))
            else:
                mutual_cond = null_latent.to(self.device)

            if do_classifier_free_guidance:
                null_mutual_cond = null_latent.to(self.device)
                mutual_cond = torch.cat([mutual_cond, mutual_cond, null_mutual_cond, null_mutual_cond], dim=0)
            elif do_classifier_free_guidance_category:
                if do_classifier_free_guidance_mutual:
                    null_mutual_cond = null_latent.to(self.device)
